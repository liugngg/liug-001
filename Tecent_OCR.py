...
def main():
    ...
    i = 0
    for file in listimg:
        result = tencentOcr.get_localimage_text(file)
        # resulTxt = (os.path.splitext(file)[0]) + '.txt'
        resultTxt = Path.WindowsPath(file).with_suffix('.txt')
        with open(resultTxt, "a", encoding='utf-8') as resfile:
            resfile.write(result)
            resfile.write('\n')
            i = i + 1
            print('第%d个文件已完成识别，结果存为：\n"%s"\n' % (i, resultTxt))

    if str.upper(isHebing) != 'Y':
        print('恭喜！所有"%s"图片文件已完成OCR' % filesufix)
        return 0
    ...
    if len(listtxt) < 2:
        print('恭喜！所有"%s"图片文件已完成OCR' % filesufix)
        print('该文件夹下没有需要合并的文件！')
        return 1
    res = str(Path.WindowsPath(listtxt[0]).with_name('result-all.txt'))
    i = 0
